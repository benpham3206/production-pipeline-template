...
# scripts
check "checkpoint.sh exists" "[ -x scripts/checkpoint.sh ]"
check "recovery.sh exists" "[ -x scripts/recovery.sh ]"

# Git checks (if in a git repo)
if [ -d .git ]; then
...
if [ -f "package.json" ]; then
  if grep -q '"test"' package.json; then
    ...
    if npm test 2>&1 | grep -qE "passed|PASS"; then
      echo "✅ Tests passing"
      PASS=$((PASS + 1))
    else
      echo "❌ Tests failing or no test results found"
      FAIL=$((FAIL + 1))
    fi
  fi
fi
